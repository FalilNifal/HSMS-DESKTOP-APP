...
function buildReceiptHtml(invoice: Invoice): string {
  ...
  const footer = invoice.invoiceFooterMessage
    ? `<div class="divider"></div><div class="footer">${esc(invoice.invoiceFooterMessage)}</div>`
    : ''
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: 'Courier New', ui-monospace, monospace; color: #000; padding: 6px 10px; width: 300px; }
...
<body>
  ...
  ${
    invoice.taxAmount > 0
      ? `<div class="row"><span class="muted">Subtotal</span><span>${esc(formatMoney(invoice.subTotal))}</span></div>
  <div class="row"><span class="muted">${esc(invoice.taxLabel)}</span><span>${esc(formatMoney(invoice.taxAmount))}</span></div>`
      : ''
  }
  <div class="row total"><span>TOTAL</span><span>${esc(formatMoney(invoice.totalAmount))}</span></div>
  ${footer}
</body>
</html>`
}
